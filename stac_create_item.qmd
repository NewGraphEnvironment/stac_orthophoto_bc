...
# Define the COG validity check
def is_geotiff_readable(url: str) -> bool:
    try:
        with rasterio.Env():
            with rasterio.open(url) as src:
                return src.profile['driver'] == 'GTiff'
    except Exception:
        return False
...
def process_item(path_item: str) -> dict | None:
    ...
    if not is_geotiff_readable(href_item):
        print(f"Skipping invalid COG: {href_item}")
        return None

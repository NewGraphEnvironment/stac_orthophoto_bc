...
keys <- ngr::ngr_s3_keys_get(
  ...
  prefix = "",
  pattern = c("orthophoto", "*.tif")
)

fs::dir_create("data")

# get rid of paths with ( in them (and dupes)
keys_clean <- unique(keys[!stringr::str_detect(keys, "\\(")])


readr::write_lines(keys_clean, "data/urls_list.txt")

fs::dir_create("/Users/airvine/Projects/gis/stac_orthophoto_bc/prod/stac_orthophoto_bc")
...
path_collection = "/Users/airvine/Projects/gis/stac_orthophoto_bc/prod/stac_orthophoto_bc/collection.json"
# test!!!!!!!!!!!!!!!
# path_collection = "/Users/airvine/Projects/gis/stac/test/collection.json"

path_s3_json = "https://stac-orthophoto-bc.s3.amazonaws.com/collection.json"
...
# Get bbox from your COGs - except that it takes forever (20 minutes - maybe bc they are not COGs - or perhaps that is just how long it takes)
# duration = timeit.timeit(lambda: bbox_combined(path_items[:500]), number=1)
# print(f"Computed bbox in {duration:.2f} seconds.")
# # bbox = bbox_combined(path_items)
...
collection = Collection(
    id=collection_id,
    description="A collection of Orthophotos from British Columbia - as served on lidarbc",
    extent=extent,
    license="CC-BY-4.0",
    title=f"Orthophotos from British Columbia - {collection_id}",
...
```{python, test-valid}
# need to load the collection.json to aws!!! first. should fix but not ging to
